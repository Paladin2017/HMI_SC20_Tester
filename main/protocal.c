...
bool protocal_is_processing = false;
bool protocal_is_packing = false;
...
int SetupPack(uint8_t *pSourceData, uint16_t SrcLen, uint8_t *pDesData) {
  ...
  while(protocal_is_packing == true) {
    vTaskDelay( 20 / portTICK_RATE_MS );
  }

  protocal_is_packing = true;
...
int Parse(uint8_t *pSourceData, uint16_t *Head, uint16_t *Tail, uint16_t buffer_size, uint8_t *pParseData) {
  ...
  while(protocal_is_processing == true) {
    vTaskDelay( 20 / portTICK_RATE_MS );
  }
  
  protocal_is_processing = true;
  ...
  while(tmp_len) {
    ...
    *Tail = tmp_tail;
    ...
    if(src_checksum != calculate_checksum) {
      ...
      tmp_len = tmp_len - 2;
      *Tail = tmp_tail;
      ESP_LOGI(TAG, "Verify fail, %02X%02X", pParseData[8], pParseData[9]);
      continue;
    }
...
/** @brief  Setup a pack of data use the protocal
  ...
  * @retval The Lenght of the pack
*/
int SetupPack2(uint8_t *pSourceData, uint16_t SrcLen, uint8_t *pDesData) {
	int i = 0;
	int j;
	uint32_t checksum;
	uint16_t tmp_u16;
...
/** @brief  Parse a pack of data use the protocal, and throw out any error datas
  ...
  * @retval The Lenght of the parsed data within the protocal head(8)
*/
int Parse2(uint8_t *pSourceData, uint16_t *Head, uint16_t *Tail, uint16_t buffer_size, uint8_t *pParseData) {
  int i;
  int tmp_tail_backup;
  uint32_t calculate_checksum;
  uint32_t src_checksum;
